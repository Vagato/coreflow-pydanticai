...
function MessageRow({ msg }: { msg: SubagentMessage }) {
  ...
  return (
    <div className={cn("rounded-lg p-3", style.bg)}>
      ...
      <p className="text-foreground/80 text-xs leading-relaxed break-words whitespace-pre-wrap">
        {msg.text}
      </p>
    </div>
  );
...
export function SubagentPanel() {
  ...
  const subagent = useResearchStore((s) => {
    ...
    return (
      s.byTurn[s.currentTurnId]?.subagents.find((a) => a.task_id === s.selectedSubagentId) ?? null
    );
  });
  ...
  const statusStyle = STATUS_STYLES[subagent.status] ?? {
