...
```bash
uv run coreflow cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run coreflow cmd rag-search "your question" --collection docs        # semantic search
uv run coreflow cmd rag-collections                                     # list collections
uv run coreflow cmd rag-stats                                           # chunk/vector counts
uv run coreflow cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run coreflow cmd rag-sources                  # list
uv run coreflow cmd rag-source-add               # add (interactive)
uv run coreflow cmd rag-source-sync --all        # trigger a sync
```
