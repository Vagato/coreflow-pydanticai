...
```bash
cd backend
uv run coreflow <group> <command> [options]
```

### Server Commands

```bash
uv run coreflow server run              # Start dev server
uv run coreflow server run --reload     # With hot reload
uv run coreflow server run --port 9000  # Custom port
uv run coreflow server routes           # Show all registered routes
```
...
```bash
uv run coreflow db init                  # Run all migrations
uv run coreflow db migrate -m "message"  # Create new migration
uv run coreflow db upgrade               # Apply pending migrations
uv run coreflow db upgrade --revision e3f  # Upgrade to specific revision
uv run coreflow db downgrade             # Rollback last migration
uv run coreflow db downgrade --revision base  # Rollback to start
uv run coreflow db current               # Show current revision
uv run coreflow db history               # Show migration history
```
...
uv run coreflow user create

# Create user non-interactively
uv run coreflow user create --email user@example.com --password secret

# Create user with specific role
uv run coreflow user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run coreflow user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run coreflow user create-admin --email admin@example.com --password secret

# Change user role
uv run coreflow user set-role user@example.com --role admin

# List all users
uv run coreflow user list
```
...
```bash
uv run coreflow cmd <command-name> [options]
```
...
uv run coreflow cmd rag-ingest ./docs/guide.pdf

# Ingest a directory
uv run coreflow cmd rag-ingest ./docs/

# Ingest recursively into a specific collection
uv run coreflow cmd rag-ingest ./docs/ --collection knowledge --recursive

# Ingest with sync mode
uv run coreflow cmd rag-ingest ./docs/ --sync-mode new_only
uv run coreflow cmd rag-ingest ./docs/ --sync-mode update_only

# Skip replacing existing documents
uv run coreflow cmd rag-ingest ./docs/ --no-replace
```
...
uv run coreflow cmd rag-search "what is fastapi"

# Search a specific collection
uv run coreflow cmd rag-search "deployment guide" --collection docs

# Get more results
uv run coreflow cmd rag-search "deployment" --top-k 10
```
...
uv run coreflow cmd rag-collections

# Show overall RAG system statistics
uv run coreflow cmd rag-stats

# Drop a collection (with confirmation)
uv run coreflow cmd rag-drop my_collection

# Drop without confirmation
uv run coreflow cmd rag-drop my_collection --yes
```
...
uv run coreflow cmd rag-sync-gdrive --collection docs

# Sync from a specific folder
uv run coreflow cmd rag-sync-gdrive --collection docs --folder-id abc123
```
...
uv run coreflow cmd rag-sync-s3 --collection docs

# Sync from a specific prefix (folder)
uv run coreflow cmd rag-sync-s3 --collection docs --prefix documents/

# Sync from a specific bucket
uv run coreflow cmd rag-sync-s3 --collection docs --bucket my-bucket
```
...
uv run coreflow cmd rag-source-remove <source-id>
uv run coreflow cmd rag-source-remove <source-id> --yes  # Skip confirmation

# Trigger sync for a specific source
uv run coreflow cmd rag-source-sync <source-id>

# Trigger sync for all active sources
uv run coreflow cmd rag-source-sync --all
```
...
```bash
uv run coreflow cmd my-command --name test
```
