...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app coreflow db upgrade
	@echo ""
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app coreflow db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app coreflow db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run coreflow db migrate -m "initial" || true
	cd backend && uv run coreflow db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend coreflow db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend coreflow db upgrade

db-downgrade:
	uv run --directory backend coreflow db downgrade

db-current:
	uv run --directory backend coreflow db current

db-history:
	uv run --directory backend coreflow db history

# === Server ===
run:
	uv run --directory backend coreflow server run --reload

run-prod:
	uv run --directory backend coreflow server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend coreflow server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend coreflow user create-admin

user-create:
	uv run --directory backend coreflow user create

user-list:
	uv run --directory backend coreflow user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/coreflow"
...
help:
	@echo ""
	@echo "coreflow - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run coreflow rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run coreflow rag-search <query> -c <collection>  Search"
	@echo "  uv run coreflow rag-collections                     List collections"
	@echo "  uv run coreflow rag-sources                         List sync sources"
	@echo "  uv run coreflow rag-source-add                      Add sync source"
	@echo "  uv run coreflow rag-source-sync <id>                Trigger sync"
	@echo ""
