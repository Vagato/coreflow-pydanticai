...
└── commands/             # Click CLI commands (auto-discovered by `coreflow cmd …`)
...
The generated project ships a Click CLI exposed as `coreflow` (after `make install`):

```bash
coreflow server run --reload          # dev server
coreflow db upgrade                   # apply migrations
coreflow db migrate -m "message"      # create new migration
coreflow user create-admin            # interactive admin creation
coreflow rag-ingest <path> -c docs    # ingest local files
coreflow rag-search "query" -c docs   # semantic search
coreflow rag-collections              # list collections
```

Run `make help` for a categorized list, or `coreflow --help` for full CLI docs.
...
coreflow rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
coreflow rag-sync-gdrive --collection documents --folder-id <id>
# Pull from S3 / MinIO
coreflow rag-sync-s3 --collection documents --prefix docs/

# Semantic search
coreflow rag-search "your query" --collection documents
```
